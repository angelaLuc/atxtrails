...
class UrbanTrails extends React.Component {
  ...
  messages = {
    ...
    NETWORK_ERROR:
      "Unable to retrieve trail data at this time, please try again later.",
  ...
  showPopup(feature, latlng) {
    this.setState({
      popup: {
        show: true,
        position: latlng,
        data: <TrailCard data={feature.properties.data} />
      }
    });
  }
  ...
  handleMapResize = () => {
    let map =
      this.mapRef.current && this.mapRef.current.leafletElement
  ...
  render() {
    ...
    if (!this.state.loading) {
      if (
        this.state.hasError ||
        (this.state.trails &&
          this.state.trails.featureColl &&
          this.state.trails.featureColl.features.length === 0)
      ) {
        const errorMessage = this.state.hasError
          ? this.messages.NETWORK_ERROR
          : this.messages.NO_DATA_FOUND;
        trailsMap = (
          <div
        ...
        const homeCoords = getHomeCoordinatesFromData(
          this.state.trails.featureColl
        );

        if (homeCoords && homeCoords.length === 2) {
          const latlng = Leaflet.latLng([
            convertLatLng(homeCoords[1]),
            convertLatLng(homeCoords[0])
          ]);
          if (latlng) {
        ...
        const popup = this.state.popup.show ? (
          <Popup position={this.state.popup.position}>
        ...
        if (this.state.filter.show) {
          filterPanel = (
            <TrailFilterPanel
              filters={this.state.filter.options}
        ...
        showTrails.features = this.state.trails.featureColl.features.reduce(
          (acc, f) => {
        ...
        trailsMap = (
          ...
          >
            ...
            <FilterControl
              ...
              selected={this.state.filter.selected}
              key={this.state.filter.geoKey + 1}
            />
            <RefreshControl
              position="topright"
