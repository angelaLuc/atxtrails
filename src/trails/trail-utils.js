...
export function getFilters (data) {
    ...
    let filteroptions = Object.keys(data.trailNames).reduce((acc, value) => {
        acc.push({ id: filterIndex += 1, value: value, selected: false, key: 'selectedFilter' });
        return acc;
    }, []);

    let sortedFilterOptions = sortByProperty(filteroptions, 'value');
    sortedFilterOptions.unshift({ id: 0, value: 'Show All', selected: true, key: 'selectedFilter' });
    return sortedFilterOptions;
}

export function urbanTrailTransform (geojson) {
    let results = geojson.reduce((acc, geo) => {
        ...
        return acc;
    }, { buildStatTypes: {}, trailNames: {}, featureColl: { type: 'FeatureCollection', features: [] } });
...
export const getHomeCoordinatesFromData = data => {
    ...
    if (data) {
        ...
        let mean = centerMean({ type: 'FeatureCollection', features: selectedFeatures }, {});
        home = mean && mean.geometry && mean.geometry.coordinates ? mean.geometry.coordinates : null;
    }
    return home;
};
